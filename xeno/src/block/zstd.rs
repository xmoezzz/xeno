    ...
    R: Read + BufRead,
{
    pub fn unpack_to(&mut self, to: impl AsRef<Path>) -> Result<(), ArchiveError> {
        ...
        zstd::stream::copy_decode(&mut self.inner, &mut writer)
            .map_err(ArchiveError::Io)?;

        Ok(())
    }

    pub fn create_with_reader(rdr: impl Read + BufRead) -> Result<ZstdBlock<impl Read + BufRead>, ArchiveError> {
        let block = ZstdBlock {
            inner: rdr,
        };
    ...
    pub fn create_with_path(path: impl AsRef<Path>) -> Result<ZstdBlock<impl Read + BufRead>, ArchiveError> {
